...
describe("Aider integration", () => {
  it("configures the documented ready notification without replacing a user command", async () => {
    ...
    expect(await readFile(join(home, ".aider.conf.yml"), "utf8")).toContain("devradar hook aider waiting_for_user");
    ...
    expect(await installAiderIntegration(customHome)).toMatchObject({ notificationConfigured: false });
