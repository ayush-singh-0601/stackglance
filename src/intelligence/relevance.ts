...
export function scoreRelevance(
  ...
  const projectMatches = [
    ...new Set(repository.technologies.map(normalize).filter((tag) => storyTags.has(tag))),
  ];
  const taskCoverage =
    taskTags.length === 0 ? 0 : taskMatches.length / Math.min(taskTags.length, 5);
  const projectCoverage =
    repository.technologies.length === 0
      ? 0
      : projectMatches.length / Math.min(repository.technologies.length, 8);
